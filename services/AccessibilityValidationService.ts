...
export class AccessibilityValidationService {
  ...
  async validateElement(element: {
    ...
    textColor?: string;
    fontSize?: number;
    images?: Array<{ altText?: string; src: string }>;
    interactive?: boolean;
    width?: number;
    height?: number;
    ...
    if (element.background && element.textColor) {
      const contrast = this.validateColorContrast(
        element.textColor,
        element.background,
        'AA',
        element.fontSize || 14
  ...
  generateAccessiblePalette(brandColor: string): AccessiblePalette {
    try {
      ...
      const primaryContrast = chroma.contrast(adjustedPrimary, background);
   ...
   */
  generateAccessibleScale(baseColor: string, steps: number = 9): string[] {
    try {
      ...
      return scale.map(color => {
        const contrastWithWhite = chroma.contrast(color, '#FFFFFF');
        const contrastWithBlack = chroma.contrast(color, '#000000');
