...
import { createDesignSystemService } from './DesignSystemService';
import type {
  DesignElement,
  ColorPalette
...
export class DesignValidationService {
  private designSystem: ReturnType<typeof createDesignSystemService>;
  private rules: ValidationRule[];

  constructor() {
    this.designSystem = createDesignSystemService();
   ...
   */
  validateDesignElement(element: DesignElement): { issues: ValidationIssue[]; score: number } {
    ...
    return [
      {
        ...
        validate: (context) => {
          ...
          return this.validateColorContrast(
            ...
            context.element.backgroundColor,
            context.element.fontSize,
            context.element.fontWeight === 'bold'
          );
        }
      },
      {
   ...
   */
  private getBrandingRules(): ValidationRule[] {
    return [
      {
      ...
      },
      {
        ...
        validate: (context) => {
          if (context.element?.fontFamily) {
            ...
            const isBrandFont = brandFonts.some(font => 
              usedFont.includes(font.toLowerCase())
            );
   ...
   */
  private getTypographyRules(): ValidationRule[] {
    return [
      {
   ...
   */
  private getColorRules(): ValidationRule[] {
    return [
      {
        id: 'color-format',
        name: 'Color Format',
   ...
   */
  private getLayoutRules(): ValidationRule[] {
    return [
      {
   ...
   */
  private isColorInPalette(color: string, palette: ColorPalette): boolean {
 ...
 */
export function createDesignValidationService(): DesignValidationService {
