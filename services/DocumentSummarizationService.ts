...
export class DocumentSummarizationService {
   ...
   */
  async createExecutiveSummary(content: string): Promise<string> {
   ...
   */
  private calculateTargetLength(originalCount: number, length: 'short' | 'medium' | 'long'): number {
    switch (length) {
      case 'short':
   ...
   */
  private formatSummary(sentences: string[], style: 'bullets' | 'paragraph' | 'highlights'): string {
    switch (style) {
      case 'bullets':
   ...
   */
  private calculateConfidence(original: string, summary: string): number {
 ...
 */
export function createDocumentSummarizationService(): DocumentSummarizationService {
