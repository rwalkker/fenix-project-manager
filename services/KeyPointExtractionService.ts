...
export interface KeyPoint {
  ...
  context?: string;
  relatedPoints?: string[]; // IDs of related points
}
...
export class KeyPointExtractionService {
   ...
   */
  async extractKeyPoints(content: string, options: ExtractionOptions = {}): Promise<KeyPoint[]> {
    const {
      maxPoints = 10,
      minImportance = 0.3,
      categories
   ...
   */
  async prioritizePoints(points: KeyPoint[]): Promise<KeyPoint[]> {
   ...
   */
  async groupByTheme(points: KeyPoint[]): Promise<PointGroup[]> {
   ...
   */
  async extractCriticalInfo(content: string): Promise<KeyPoint[]> {
   ...
   * Extract main ideas
   */
  async extractMainIdeas(content: string, count: number = 3): Promise<KeyPoint[]> {
    return this.extractKeyPoints(content, {
   ...
   * Extract insights
   */
  async extractInsights(content: string): Promise<KeyPoint[]> {
    return this.extractKeyPoints(content, {
      categories: ['insight'],
      minImportance: 0.5,
      maxPoints: 5
  ...
  private analyzeSentence(sentence: string, fullContent: string): KeyPoint | null {
  ...
  private categorize(sentence: string): KeyPoint['category'] {
  ...
  private isMainIdea(sentence: string): boolean {
    const lower = sentence.toLowerCase();
  ...
  private calculateImportance(sentence: string, fullContent: string): number {
    let score = 0.5; // Base score

    // Length factor (longer sentences often more important)
    if (sentence.length > 100) score += 0.1;
    if (sentence.length > 150) score += 0.1;
    ...
    if (position < 0.2) score += 0.1; // First 20%
    if (position > 0.8) score += 0.05; // Last 20% (conclusions)

    // Numbers and data (often important)
    if (/\d+/.test(sentence)) score += 0.05;
    if (/%/.test(sentence)) score += 0.05;

    return Math.min(score, 1.0);
  }

  private calculatePriority(point: KeyPoint, _allPoints: KeyPoint[]): number {
    let priority = point.importance;

    // Boost for critical info and action items
    if (point.category === 'critical-info') priority += 0.2;
    if (point.category === 'action-item') priority += 0.15;
    if (point.category === 'main-idea') priority += 0.1;
  ...
  private extractContext(sentence: string, fullContent: string): string {
  ...
  private findRelatedPoints(points: KeyPoint[]): void {
  ...
  private areRelated(point1: KeyPoint, point2: KeyPoint): boolean {
  ...
  private createActionItem(sentence: string): ActionItem {
    return {
  ...
  private determinePriority(sentence: string): ActionItem['priority'] {
    const lower = sentence.toLowerCase();
  ...
  private identifyThemes(points: KeyPoint[]): string[] {
  ...
  private belongsToTheme(point: KeyPoint, theme: string): boolean {
