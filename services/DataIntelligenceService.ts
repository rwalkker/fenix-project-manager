...
export class DataIntelligenceService {
   ...
   */
  analyzeColumn(data: any[], columnName: string = 'Column'): DataAnalysis {
   ...
   * Detect data type
   */
  private detectDataType(data: any[]): DataAnalysis['type'] {
    ...
    if (Object.keys(typeCounts).length === 0) return 'text';
    if (Object.keys(typeCounts).length > 1) return 'mixed';

    const dominantType = _.maxBy(
      Object.keys(typeCounts),
      key => typeCounts[key]
    );

    return dominantType as DataAnalysis['type'];
  }
   ...
   */
  private calculateStatistics(data: any[]): DataAnalysis['statistics'] {
    const numbers = data
   ...
   */
  private detectOutliers(numbers: number[], q1: number, q3: number): number[] {
   ...
   * Assess data quality
   */
  private assessDataQuality(data: any[]): DataAnalysis['quality'] {
   ...
   */
  private detectPatterns(data: any[]): DataAnalysis['patterns'] {
    const numbers = data.filter(v => typeof v === 'number' && !isNaN(v));
   ...
   */
  private calculateTrendCorrelation(data: number[]): number {
    const n = data.length;
   ...
   */
  private detectSeasonality(numbers: number[]): boolean {
    ...
    for (const period of periods) {
      if (numbers.length < period * 2) continue;
      
      const chunks = _.chunk(numbers, period);
  ...
  private generateSuggestions(
    type: string,
    data: any[],
    quality: DataAnalysis['quality']
  ): DataAnalysis['suggestions'] {
    const suggestions: DataAnalysis['suggestions'] = {
      chartType: 'column',
      formatting: 'general',
      validation: 'none',
      aggregation: 'sum'
  ...
  suggestFormulas(analysis: DataAnalysis): FormulaResult[] {
    const suggestions: FormulaResult[] = [];

    if (analysis.type === 'numeric' && analysis.statistics) {
      suggestions.push(
        this.generateFormula('sum'),
        this.generateFormula('average'),
        this.generateFormula('max'),
        this.generateFormula('min')
      );
