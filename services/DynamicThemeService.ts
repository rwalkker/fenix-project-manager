...
import { getBedrockService } from './AWSBedrockService';
...
export class DynamicThemeService {
  ...
  private themeHistory: Map<string, DynamicThemeConfig> = new Map();
  private defaultTheme: DynamicThemeConfig;
   ...
   */
  async generateDynamicTheme(request: ThemeGenerationRequest): Promise<DynamicThemeConfig> {
   ...
   */
  private async generateAITheme(request: ThemeGenerationRequest): Promise<DynamicThemeConfig> {
...
Format as JSON:
{
  ...
  "chartColors": ["#HEX1", "#HEX2", "#HEX3", "#HEX4", "#HEX5"]
    ...
    const themeConfig: DynamicThemeConfig = {
   ...
   */
  async generateThemeSuggestions(contentText: string, title: string): Promise<ThemeGenerationRequest[]> {
...
[
   ...
   */
  private findSimilarTheme(request: ThemeGenerationRequest): DynamicThemeConfig | null {
   ...
   */
  private calculateThemeSimilarity(theme: DynamicThemeConfig, request: ThemeGenerationRequest): number {
    let score = 0;
    
    // Check description for content type and mood keywords
    const description = theme.description.toLowerCase();
    if (description.includes(request.contentType)) score += 0.3;
    if (description.includes(request.mood)) score += 0.3;
    if (request.industry && description.includes(request.industry.toLowerCase())) score += 0.2;
   ...
   */
  private selectFont(contentType: string, fontType: 'title' | 'body'): string {
   ...
   */
  private selectChartStyle(mood: string): 'modern' | 'classic' | 'vibrant' | 'minimal' {
   ...
   */
  private selectIconSet(iconStyle: string): string {
   ...
   */
  private createAmazonDefaultTheme(): DynamicThemeConfig {
    return {
      id: 'amazon-default',
 ...
 */
export function createDynamicThemeService(): DynamicThemeService {
