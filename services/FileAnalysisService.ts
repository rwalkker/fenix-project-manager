...
export class FileAnalysisService {
   ...
   */
  public async analyzeFile(filePath: string, originalName: string): Promise<string> {
    try {
   ...
   */
  private async analyzeTextFile(filePath: string, originalName: string): Promise<string> {
    try {
   ...
   * Analyze CSV files
   */
  private async analyzeCsvFile(filePath: string, originalName: string): Promise<string> {
    try {
      const content = fs.readFileSync(filePath, 'utf8');
      ...
      if (dataRows > 0 && lines[1]) {
        ...
        if (hasNumbers) analysis += ' - Contains numerical data';
        if (hasDates) analysis += ' - Contains date data';
      }
   ...
   * Analyze JSON files
   */
  private async analyzeJsonFile(filePath: string, originalName: string): Promise<string> {
    try {
      const content = fs.readFileSync(filePath, 'utf8');
   ...
   */
  private analyzePdfFile(originalName: string, fileSize: number): string {
   ...
   */
  private analyzeWordFile(originalName: string, fileSize: number): string {
   ...
   */
  private analyzeExcelFile(originalName: string, fileSize: number): string {
   ...
   */
  private analyzePowerPointFile(originalName: string, fileSize: number): string {
   ...
   */
  public async getFileContentForAI(filePath: string, originalName: string): Promise<string | null> {
    try {
      const extension = path.extname(originalName).toLowerCase();
   ...
   */
  public getFileMetadataForAI(filePath: string, originalName: string): any {
    try {
