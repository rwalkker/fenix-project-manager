...
export class SentimentAnalysisService {
  ...
  async analyzeTone(content: string): Promise<ToneAnalysis> {
    ...
    const primary = sorted[0][0] as ToneType;
    const secondary = sorted[1][1] > 0.3 ? sorted[1][0] as ToneType : undefined;

    // Add characteristics
    if (scores.formal > 0.6) characteristics.push('Professional language');
    if (scores.technical > 0.5) characteristics.push('Technical terminology');
    if (scores.persuasive > 0.5) characteristics.push('Persuasive elements');
    if (scores.emotional > 0.4) characteristics.push('Emotional language');
   ...
   */
  async identifyEmotionalLanguage(content: string): Promise<EmotionalLanguage> {
    const words = this.tokenize(content);
   ...
   */
  async analyze(content: string, audienceType?: string): Promise<SentimentAnalysisResult> {
   ...
   */
  async suggestToneAdjustments(content: string, options: ToneAdjustmentOptions): Promise<string[]> {
   ...
   */
  async flagProblematicLanguage(content: string): Promise<string[]> {
    const flags: string[] = [];
  ...
  private calculateFormalityScore(content: string): number {
    ...
    // Formal indicators
    if (lower.includes('therefore') || lower.includes('furthermore')) score += 0.2;
    if (lower.includes('hereby') || lower.includes('pursuant')) score += 0.2;
    if (!lower.includes("'")) score += 0.1; // No contractions

    // Informal indicators
    if (lower.includes("'ll") || lower.includes("'ve")) score -= 0.2;
    if (lower.includes('gonna') || lower.includes('wanna')) score -= 0.3;

    return Math.max(0, Math.min(1, score));
  }

  private calculateCasualityScore(content: string): number {
  ...
  private calculatePersuasivenessScore(content: string): number {
  ...
  private calculateInformativenessScore(content: string): number {
    let score = 0.5;
    // Check for data and facts
    if (/\d+/.test(content)) score += 0.2;
    if (/%/.test(content)) score += 0.1;
    if (content.includes('according to') || content.includes('research shows')) score += 0.2;
  ...
  private categorizeEmotions(emotionalWords: string[]): EmotionalLanguage['emotions'] {
    const emotions = { joy: 0, anger: 0, sadness: 0, fear: 0, surprise: 0 };
    
    emotionalWords.forEach(word => {
      const lower = word.toLowerCase();
      if (['happy', 'excited', 'delighted', 'pleased', 'love'].includes(lower)) emotions.joy++;
      if (['angry', 'frustrated', 'hate', 'upset'].includes(lower)) emotions.anger++;
      if (['sad', 'disappointed', 'unfortunate'].includes(lower)) emotions.sadness++;
      if (['worried', 'anxious', 'concerned'].includes(lower)) emotions.fear++;
      if (['surprised', 'amazed', 'shocked'].includes(lower)) emotions.surprise++;
    });
