...
exports.CitationManagementService = void 0;
/**
...
class CitationManagementService {
     ...
     */
    async addReference(sourceId, location, context) {
     ...
     */
    async getReferences(sourceId) {
        return this.references.filter(ref => ref.sourceId === sourceId);
    }
    /**
     * Generate bibliography
     */
    async generateBibliography(style = 'APA') {
        const entries = [];
     ...
     */
    async formatCitation(sourceId, style = 'APA') {
        const source = this.sources.get(sourceId);
        if (!source) {
            throw new Error(`Source not found: ${sourceId}`);
        }
     ...
     */
    async validateCitation(sourceId) {
        const source = this.sources.get(sourceId);
        if (!source) {
            return {
                valid: false,
        ...
        switch (source.type) {
            ...
            case 'journal':
                if (!source.volume)
                    suggestions.push('Consider adding volume number');
                if (!source.pages)
                    suggestions.push('Consider adding page numbers');
                break;
            case 'website':
                if (!source.url)
                    missingFields.push('url');
                if (!source.accessDate)
                    suggestions.push('Consider adding access date');
                break;
        }
        // Check for DOI
    ...
    formatCitationByStyle(source, style) {
        switch (style) {
            case 'APA':
...
exports.CitationManagementService = CitationManagementService;
//# sourceMappingURL=CitationManagementService.js.map