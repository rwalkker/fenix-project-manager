...
exports.PowerPointAgent = void 0;
exports.createPowerPointAgent = createPowerPointAgent;
...
const AccessibilityValidationService_1 = require("../services/AccessibilityValidationService");
const SmartLayoutService_1 = require("../services/SmartLayoutService");
const ValidationService_1 = require("../services/ValidationService");
const DynamicThemeService_1 = require("../services/DynamicThemeService");
/**
 * PowerPoint Agent
...
class PowerPointAgent {
    ...
    currentDynamicTheme;
    constructor(theme) {
        this.theme = theme || powerpoint_types_1.AMAZON_POWERPOINT_THEME;
        this.outputDir = process.env.STORAGE_PATH || './output/powerpoint';
        this.bedrock = (0, AWSBedrockService_1.getBedrockService)();
        this.accessibilityService = new AccessibilityValidationService_1.AccessibilityValidationService();
        this.layoutService = new SmartLayoutService_1.SmartLayoutService();
        this.validationService = new ValidationService_1.ValidationService();
        this.dynamicThemeService = new DynamicThemeService_1.DynamicThemeService();
    ...
    async generate(options) {
        ...
        try {
            ...
            if (!options.theme || options.theme === powerpoint_types_1.AMAZON_POWERPOINT_THEME) {
     ...
     */
    async generateDynamicTheme(options) {
        try {
     ...
     */
    applyDynamicTheme(pptx, dynamicTheme) {
     ...
     */
    async createDynamicSlide(pptx, slideDefinition, slideNumber, attachedFiles) {
     ...
     * Apply dynamic background based on theme configuration
     */
    applyDynamicBackground(slide, slideType, dynamicTheme) {
     ...
     */
    addPatternElements(slide, dynamicTheme) {
     ...
     */
    addGeometricElements(slide, dynamicTheme) {
     ...
     */
    addDynamicVisualElements(slide, slideType, dynamicTheme, slideNumber) {
        if (!dynamicTheme)
            return;
     ...
     */
    addBoldAccents(slide, slideType, dynamicTheme) {
     ...
     */
    addCreativeAccents(slide, slideType, dynamicTheme) {
     ...
     */
    addMinimalAccents(slide, slideType, dynamicTheme) {
        // Minimal line accent
        slide.addShape('rect', {
            x: '5%',
            y: '15%',
     ...
     */
    addCorporateAccents(slide, slideType, dynamicTheme) {
     ...
     */
    addDynamicWidgets(slide, widgets, slideType) {
     ...
     */
    addWidget(slide, widget, slideType) {
        switch (widget.type) {
            case 'progress-bar':
     ...
     */
    addProgressBarWidget(slide, widget) {
        const dynamicTheme = this.currentDynamicTheme;
        if (!dynamicTheme)
            return;
     ...
     */
    addMetricCardWidget(slide, widget) {
        const dynamicTheme = this.currentDynamicTheme;
        if (!dynamicTheme)
            return;
     ...
     */
    createDynamicTitleSlide(slide, definition, dynamicTheme) {
     ...
     */
    createDynamicSectionSlide(slide, definition, dynamicTheme) {
     ...
     */
    createDynamicContentSlide(slide, definition, dynamicTheme) {
     ...
     */
    createDynamicTwoColumnSlide(slide, definition, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
        // Title
     ...
     */
    createDynamicChartSlide(slide, definition, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
        // Title
        ...
        // Find chart content
        const chartContent = definition.content?.find(c => c.type === 'chart');
        if (chartContent) {
     ...
     */
    createDynamicTableSlide(slide, definition, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
        // Title
        ...
        // Find table content
        const tableContent = definition.content?.find(c => c.type === 'table');
        if (tableContent) {
     ...
     */
    createDynamicQuoteSlide(slide, definition, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
        const quoteContent = definition.content?.find(c => c.type === 'quote');
        if (quoteContent) {
     ...
     */
    createDynamicAgendaSlide(slide, definition, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
        // Title
        ...
        // Agenda items
        const bulletContent = definition.content?.find(c => c.type === 'bullets');
        if (bulletContent) {
     ...
     */
    createDynamicThankYouSlide(slide, definition, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
     ...
     */
    addDynamicContent(slide, content, yOffset, dynamicTheme, xPos = '5%', width = '90%') {
        switch (content.type) {
            case 'text':
                return this.addDynamicText(slide, content, yOffset, dynamicTheme, xPos, width);
            case 'bullets':
                return this.addDynamicBullets(slide, content, yOffset, dynamicTheme, xPos, width);
            case 'image':
                return this.addImage(slide, content, yOffset);
            case 'chart':
                return this.addDynamicChart(slide, content, yOffset, dynamicTheme);
            case 'table':
                return this.addDynamicTable(slide, content, yOffset, dynamicTheme);
            default:
                return yOffset;
        }
    }
     ...
     */
    addDynamicText(slide, content, yOffset, dynamicTheme, xPos = '5%', width = '90%') {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
     ...
     */
    addDynamicBullets(slide, content, yOffset, dynamicTheme, xPos = '5%', width = '90%') {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const fonts = theme?.fonts || this.theme.fonts;
     ...
     */
    addDynamicChart(slide, content, yOffset, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
     ...
     */
    addDynamicTable(slide, content, yOffset, dynamicTheme) {
        const theme = dynamicTheme || this.currentDynamicTheme;
        const colors = theme?.colors || this.theme.colors;
        const rows = [
     ...
     */
    createTwoColumnSlide(slide, definition) {
        // Title
        if (definition.title) {
            slide.addText(definition.title, {
                x: '5%',
     ...
     * Create chart slide
     */
    createChartSlide(slide, definition) {
        // Title
        if (definition.title) {
            slide.addText(definition.title, {
                x: '5%',
     ...
     * Create table slide
     */
    createTableSlide(slide, definition) {
        // Title
        if (definition.title) {
            slide.addText(definition.title, {
                x: '5%',
     ...
     * Create quote slide
     */
    createQuoteSlide(slide, definition) {
        const quoteContent = definition.content?.find(c => c.type === 'quote');
        if (quoteContent) {
            // Quote text
            slide.addText(`"${quoteContent.text}"`, {
     ...
     * Create agenda slide
     */
    createAgendaSlide(slide, definition) {
        // Title
        slide.addText('Agenda', {
            x: '5%',
            y: '5%',
     ...
     */
    createThankYouSlide(slide, definition) {
        slide.addText('Thank You', {
            x: '10%',
            y: '40%',
            w: '80%',
     ...
     */
    addContent(slide, content, yOffset, xPos = '5%', width = '90%') {
        switch (content.type) {
            case 'text':
                return this.addText(slide, content, yOffset, xPos, width);
            case 'bullets':
                return this.addBullets(slide, content, yOffset, xPos, width);
            case 'image':
                return this.addImage(slide, content, yOffset);
            case 'chart':
                return this.addChart(slide, content, yOffset);
            case 'table':
                return this.addTable(slide, content, yOffset);
            default:
                return yOffset;
        }
    }
     ...
     */
    addText(slide, content, yOffset, xPos = '5%', width = '90%') {
        slide.addText(content.text, {
            x: xPos,
            y: `${yOffset}%`,
            w: width,
     ...
     */
    addBullets(slide, content, yOffset, xPos = '5%', width = '90%') {
     ...
     * Add image
     */
    addImage(slide, content, yOffset) {
        try {
            slide.addImage({
                path: content.path,
                x: (content.position?.x || '20%'),
                y: (content.position?.y || `${yOffset}%`),
                w: (content.size?.width || '60%'),
                h: (content.size?.height || '40%')
            });
            return yOffset + 45;
        }
        catch (error) {
     ...
     * Add chart
     */
    addChart(slide, content, yOffset) {
     ...
     * Add table
     */
    addTable(slide, content, yOffset) {
        const rows = [
     ...
     */
    async processAttachedFiles(attachedFiles) {
        if (!attachedFiles || attachedFiles.length === 0) {
            return '';
        }
        const fileAnalysisService = new (await Promise.resolve().then(() => __importStar(require('../services/FileAnalysisService')))).FileAnalysisService();
     ...
     */
    async generateSlideContent(topic, slideType, attachedFiles) {
...
Format as JSON:
{
  ...
  "fileReferences": ["filename1", "filename2"] // files referenced in content
     ...
     */
    async generateSampleChart(topic, chartType = 'column') {
...
Format as JSON:
{
  ...
  "unit": "%" or "$M" or "units" etc.
        ...
        try {
            ...
            return {
                type: 'chart',
                chartType: chartType, // Use any to avoid ChartType import issues
                title: data.title,
                data: {
                    labels: data.labels,
                    datasets: [{
     ...
     */
    async optimizePresentation(slides) {
...
Format as JSON array: ["suggestion 1", "suggestion 2", ...]`;
    ...
    async generateFromTemplate(templateId, title, data) {
        ...
        try {
            ...
            const { getPowerPointTemplate } = await Promise.resolve().then(() => __importStar(require('../templates/powerpoint-templates')));
            // Get the template
            const templateOptions = getPowerPointTemplate(templateId, {
                ...data,
                projectName: title,
                title: title
            });
     ...
     */
    async generatePresentation(inputs) {
     ...
     */
    async generateProcessMap(options) {
     ...
     */
    async generateFishboneDiagram(options) {
        const pres = new pptxgenjs_1.default();
        // Apply theme
        this.applyTheme(pres, this.theme);
        // Create title slide
...
exports.PowerPointAgent = PowerPointAgent;
/**
 * Create PowerPoint agent instance
 */
function createPowerPointAgent(theme) {
    return new PowerPointAgent(theme);
}
//# sourceMappingURL=PowerPointAgent.js.map