...
exports.IntelligentDocumentService = void 0;
exports.createIntelligentDocumentService = createIntelligentDocumentService;
/**
...
class IntelligentDocumentService {
     ...
     */
    async trackRelationship(sourceId, targetId, type, sharedContent = []) {
        const relationship = {
            id: generateId(),
     ...
     */
    getWorkflowTemplatesByCategory(category) {
        return Array.from(this.workflowTemplates.values())
     ...
     * Determine content type
     */
    determineContentType(_text, keywords) {
        // Check for presentation indicators
        const presentationScore = keywords.filter(k => ['status', 'update', 'executive', 'review', 'presentation'].includes(k)).length;
        // Check for data analysis indicators
        const dataScore = keywords.filter(k => ['kpi', 'metrics', 'data', 'analytics', 'statistics', 'trends'].includes(k)).length;
        // Check for documentation indicators
        const docScore = keywords.filter(k => ['documentation', 'specification', 'technical', 'plan'].includes(k)).length;
     ...
     */
    isNarrativeFocused(lowerText, keywords) {
     ...
     * Suggest formats based on analysis
     */
    suggestFormats(type, dataIntensive, visualHeavy, narrativeFocused) {
        const formats = [];
     ...
     */
    findBestTemplate(format, analysis) {
        ...
        const scored = candidates.map(template => {
            let score = 0;
            // Match keywords
            const matchingKeywords = template.keywords.filter(k => analysis.keywords.includes(k));
     ...
     */
    calculateConfidence(analysis, format) {
     ...
     */
    generateRationale(analysis, format, templateId) {
     ...
     * Suggest supporting documents
     */
    suggestSupportingDocuments(analysis, primaryFormat) {
        const supporting = [];
     ...
     */
    buildWorkflowSteps(recommendation, inputs) {
        const steps = [];
     ...
     */
    buildWorkflowDependencies(steps) {
        const dependencies = [];
     ...
     */
    createWorkflowFromTemplate(template, request) {
...
exports.IntelligentDocumentService = IntelligentDocumentService;
/**
 * Create Intelligent Document Service instance
 */
function createIntelligentDocumentService() {
    return new IntelligentDocumentService();
}
//# sourceMappingURL=IntelligentDocumentService.js.map