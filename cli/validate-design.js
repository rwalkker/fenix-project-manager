...
exports.DesignValidationCLI = void 0;
const DesignValidationService_1 = require("../services/DesignValidationService");
...
(function (Command) {
    ...
    Command["CHECK_CONTRAST"] = "check-contrast";
    Command["CHECK_ACCESSIBILITY"] = "check-accessibility";
    Command["CHECK_BRAND"] = "check-brand";
...
class DesignValidationCLI {
    ...
    designSystem;
    constructor() {
        this.validationService = new DesignValidationService_1.DesignValidationService();
        this.designSystem = new DesignSystemService_1.DesignSystemService();
    }
    /**
     * Run CLI
     */
    ...
    async checkContrast(options) {
        ...
        for (const element of design.elements) {
            if (element.color && element.backgroundColor) {
                const contrastRatio = this.designSystem.getContrastRatio(element.color, element.backgroundColor);
     ...
     */
    async checkBrand(options) {
        console.log('🏢 Checking brand compliance...\n');
        const design = await this.loadDesign(options.input);
        const issues = [];
        for (const element of design.elements) {
            const result = this.validationService.validateDesignElement(element);
            const brandIssues = (result.issues || []).filter(issue => issue.category === 'branding');
...
`);
     ...
     */
    async saveResults(_results, output, _format) {
     ...
     * Save design to file
     */
    async saveDesign(_design, output) {
        // Implementation would save to file
     ...
     */
    displayValidationResults(results, overallScore, options) {
...
exports.DesignValidationCLI = DesignValidationCLI;
/**
 * Main entry point
 */
...
//# sourceMappingURL=validate-design.js.map