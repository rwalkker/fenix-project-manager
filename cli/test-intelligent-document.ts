...
import type { WorkflowCreationRequest } from '../models/orchestration-types';
...
async function testIntelligentDocumentService() {
  ...
  const service = new IntelligentDocumentService();
  ...
  const workflowRequest: WorkflowCreationRequest = {
  ...
  const templateRequest: WorkflowCreationRequest = {
  ...
  await service.trackRelationship(
    'doc-1',
    'doc-2',
    'supports',
