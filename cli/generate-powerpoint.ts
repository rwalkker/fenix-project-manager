...
import { createPowerPointAgent } from '../agents/PowerPointAgent';
...
program
  ...
  .action(() => {
    ...
    const templates = listPowerPointTemplates();
...
program
  ...
  .action(async (template: string, options: any) => {
    try {
      ...
      if (options.projectName) customData.projectName = options.projectName;
      if (options.topic) customData.topic = options.topic;
      if (options.presenter) customData.presenter = options.presenter;
      if (options.trainer) customData.trainer = options.trainer;
      if (options.changeName) customData.changeName = options.changeName;
      if (options.systemName) customData.systemName = options.systemName;
      if (options.quarter) customData.quarter = options.quarter;
      if (options.department) customData.department = options.department;

      // Get template
      const presentationOptions = getPowerPointTemplate(template as PowerPointTemplateType, customData);

      // Create agent and generate
      const agent = createPowerPointAgent();
...
program
  ...
  .action(async (template: string) => {
    try {
      ...
      const presentationOptions = getPowerPointTemplate(template as PowerPointTemplateType, data);
      const agent = createPowerPointAgent();
      const result = await agent.generate(presentationOptions);
