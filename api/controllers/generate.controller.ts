...
import { getWebSocketService } from '../server';
import { getBedrockService } from '../../services/AWSBedrockService';
import { getMockAIService } from '../../services/MockAIService';
import { getDocumentStorageService } from '../../services/DocumentStorageService';

// Import agents
import { PowerPointAgent } from '../../agents/PowerPointAgent';
import { ExcelAgent } from '../../agents/ExcelAgent';
import { WordAgent } from '../../agents/WordAgent';
import { AgentOrchestrator } from '../../services/AgentOrchestrator';
import type { BulletListContent, TextContent } from '../../models/powerpoint-types';
...
export class GenerateController {
  private storageService = getDocumentStorageService();
   ...
   */
  async generatePowerPoint(req: Request, res: Response): Promise<void> {
    ...
    if (attachedFiles && attachedFiles.length > 0) {
      console.log(`[PowerPoint] Processing ${attachedFiles.length} attached files:`, 
    ...
    const jobId = await this.storageService.createDocument(
      'powerpoint',
      title,
      user.userId,
      user.username,
   ...
   */
  private async generatePowerPointAsync(
    jobId: string,
    title: string,
    template?: string,
    content?: any,
   ...
   */
  private async generateWithFullAIOrchestrationAndDynamicTheming(
   ...
   */
  private async generateWithTemplateAndDynamicTheming(
    orchestrator: AgentOrchestrator,
    template: string,
    title: string,
    content: any,
   ...
   */
  private async generateWithExplicitSlidesAndDynamicTheming(
    orchestrator: AgentOrchestrator,
    title: string,
    slides: any[],
    themePreferences: any,
   ...
   */
  private async generateEnhancedSlideContent(
...
Format as JSON:
{
   ...
   */
  private async generateVisualEnhancements(
    visualAgent: any, // Use any to avoid import issues
    slide: any,
    topic: string
  ): Promise<any> {
    try {
      ...
      const elements = [
        ...
        },
        {
          id: 'slide-content',
          type: 'text' as const,
          content: slide.content?.[0]?.items?.map((item: any) => item.text).join(' ') || '',
          fontSize: 16,
          foreground: '#000000'
        }
      ];
   ...
   */
  private async generateWithExplicitSlides(
    orchestrator: AgentOrchestrator,
    title: string,
    slides: any[],
    jobId: string,
   ...
   */
  private async generateExcelAsync(
    jobId: string,
    title: string,
    template?: string,
    data?: any,
   ...
   */
  private async generateWordAsync(
    jobId: string,
    title: string,
    template?: string,
    content?: any,
      ...
      if (template) {
        ...
        for (let i = 0; i < outline.length; i++) {
          try {
            ...
            const sectionContent = await agent.generateContent(
              `${outline[i]}: ${contentText}`,
  ...
  async listDocuments(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const documents = await this.storageService.listDocuments(
        user.userId,
        user.role,
      ...
      throw new ApiError(500, 'Failed to list documents: ' + error.message);
    }
  }
   ...
   */
  async generateProcessMap(req: Request, res: Response): Promise<void> {
   ...
   */
  async generateFishboneDiagram(req: Request, res: Response): Promise<void> {
   ...
   */
  async generateFiveWhys(req: Request, res: Response): Promise<void> {
    const { title, problemStatement, categories, options } = req.body;
   ...
   */
  private async generateProcessMapAsync(
    jobId: string,
    title: string,
    ...
    try {
      ...
      const { getProcessMapTemplate } = await import('../../templates/process-improvement-templates');
   ...
   */
  private async generateFishboneDiagramAsync(
    jobId: string,
    title: string,
    problemStatement: string,
    categories?: any[],
    ...
    try {
      ...
      // Import process improvement templates
      const { getFishboneTemplate } = await import('../../templates/process-improvement-templates');
   ...
   */
  private async generateFiveWhysAsync(
    jobId: string,
    title: string,
    problemStatement: string,
    categories?: any[],
    ...
    try {
      ...
      // Import process improvement templates
      const { getFiveWhysTemplate } = await import('../../templates/process-improvement-templates');
   ...
   */
  async listRecentFiles(req: Request, res: Response): Promise<void> {
    const user = req.user;
    if (!user) {
      throw new ApiError(401, 'Authentication required');
    }
    ...
    try {
      const outputDirs = [
        ...
        { path: path.join(this.outputDir, 'powerpoint'), type: 'powerpoint' },
        { path: path.join(this.outputDir, 'excel'), type: 'excel' },
        { path: path.join(this.outputDir, 'word'), type: 'word' }
      ];
      ...
      for (const dir of outputDirs) {
        ...
        for (const file of files) {
          if (!file.endsWith('.pptx') && !file.endsWith('.xlsx') && !file.endsWith('.docx')) continue;
      ...
      throw new ApiError(500, 'Failed to list recent files');
    }
  }
   ...
   */
  async downloadByFilename(req: Request, res: Response): Promise<void> {
    ...
    try {
      const outputDirs = [
        this.outputDir,
        path.join(this.outputDir, 'powerpoint'),
        path.join(this.outputDir, 'excel'),
        path.join(this.outputDir, 'word')
      ];
      ...
      if (!isRecent && user.role !== 'admin') {
        throw new ApiError(403, 'Access denied: File is too old');
      }
   ...
   */
  async getThemeSuggestions(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const { DynamicThemeService } = await import('../../services/DynamicThemeService');
      ...
      throw new ApiError(500, 'Failed to generate theme suggestions: ' + error.message);
    }
  }
   ...
   * Get theme history
   */
  async getThemeHistory(req: Request, res: Response): Promise<void> {
    const user = req.user;
    ...
    try {
      ...
      throw new ApiError(500, 'Failed to get theme history: ' + error.message);
    }
  }
   ...
   * Rate a theme
   */
  async rateTheme(req: Request, res: Response): Promise<void> {
    ...
    if (!themeId || typeof rating !== 'number' || rating < 1 || rating > 5) {
      throw new ApiError(400, 'Valid theme ID and rating (1-5) required');
    }
    ...
    try {
      ...
      throw new ApiError(500, 'Failed to rate theme: ' + error.message);
    }
  }
   ...
   */
  private async generateSlideTopics(title: string, content: string, count: number): Promise<string[]> {
